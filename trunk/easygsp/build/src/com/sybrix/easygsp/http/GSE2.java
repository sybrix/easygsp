 ...
 */
public class GSE2 implements ResourceConnector {
        ...
        private static class ScriptCacheEntry {
                ...
                private Set dependencyClassNameSet = new HashSet();
        }
        ...
        private class ScriptClassLoader extends GroovyClassLoader {
                ...
                protected Class findClass(String className) throws ClassNotFoundException {
                        ...
                        try {
                                ...
                                Class clazz = parseClass(dependentScriptConn.getInputStream(), filename);
                                StaticControllerMethods.addMethods(clazz);
                                currentCacheEntry.dependencyClassNameSet.add(className);
         ...
         */
        public GSE2(URL[] roots) {
                this.roots = roots;
                this.rc = this;
                initGroovyLoader(getClass().getClassLoader());
        }

        public GSE2(URL[] roots, ClassLoader parentClassLoader) {
                this(roots);
                initGroovyLoader(parentClassLoader);
        }

        public GSE2(String[] urls) throws IOException {
        ...
        public GSE2(String[] urls, ClassLoader parentClassLoader) throws IOException {
                this(urls);
                initGroovyLoader(parentClassLoader);
        }

        public GSE2(String url) throws IOException {
                this(new String[]{url});
        }

        public GSE2(String url, ClassLoader parentClassLoader) throws IOException {
                this(url);
                initGroovyLoader(parentClassLoader);
        }

        public GSE2(ResourceConnector rc) {
                this.rc = rc;
                initGroovyLoader(getClass().getClassLoader());
        }

        public GSE2(ResourceConnector rc, ClassLoader parentClassLoader) {
                this(rc);
                initGroovyLoader(parentClassLoader);
        }
        ...
        private ScriptCacheEntry updateCacheEntry(String scriptName)
                ...
                synchronized (scriptName) {
                        ...
                        if (entry == null || entry.lastModified < lastModified || dependencyOutOfDate(entry)) {
                                // Make a new entry
                                removeAlldependencies(groovyLoader, entry);
                                ...
                                StaticControllerMethods.addMethods(currentCacheEntry.scriptClass);
         ...
         * @throws ScriptException   if there is a problem parsing the script
         */
        private ScriptCacheEntry updateCacheEntry2(String scriptName)
                throws ResourceException, ScriptException {
                ScriptCacheEntry entry;
...
//                                                }
//
//                                                in = new BufferedInputStream(new ByteArrayInputStream(file.toString().getBytes()));
//                                                //in.reset();
//                                        } else {
                                                in = groovyScriptConn.getInputStream();
//                                        }
...
//                                        }
        ...
        private boolean removeAlldependencies(GroovyClassLoader groovyClassLoader, ScriptCacheEntry entry) {
                if (entry != null) {
                        for (Iterator i = entry.dependencyClassNameSet.iterator(); i.hasNext();) {

                               // groovyClassLoader.removeClassCacheEntry(i.next().toString());
                        }
                }
                return false;
        }
