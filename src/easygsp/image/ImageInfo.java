 ...
 * <pre>
 * sybrix.image.ImageInfo ii = new sybrix.image.ImageInfo();
 * ii.setInput(bin); // bin can be InputStream or RandomAccessFile
 ...
 * <pre>
 *   java sybrix.image.ImageInfo *.jpg *.png *.gif http://somesite.tld/image.jpg
 * </pre>
 * or call it without parameters and pipe data to it:
 * <pre>
 *   java sybrix.image.ImageInfo &lt; image.jpg
 * </pre>
 * <p>
 * Known limitations:
 * <ul>
 ...
 * <li>Transparency information is not included bin the bits per pixel count.
 ...
 * <li><strong>2001-10-16</strong> Fixed bug bin read(int[], int, int) that returned
 ...
 * <li><strong>2002-03-15</strong> Added support to recognize number of images bin file. Only works with GIF.
 ...
 * <li><strong>2002-04-10</strong> Fixed a bug bin the feature 'determine number of images bin animated GIF' introduced with version 1.1.
 *   Thanks to Marcelo P. Lima for sending bin the bug report.
 ...
 *  stored bin an internal list when encountered bin an input image file / stream.
 *  Added two methods to return the physical width and height of the image bin dpi:
 ...
 * <li><strong>2002-06-17</strong> Added support for SWF, sent bin by Michael Aird.
 ...
 *  Less bytes than necessary may have been skipped, leading to flaws bin the retrieved information bin some cases.
 ...
 *  interlaced PNG and GIF. A new method {@link #isProgressive()} returns whether sybrix.image.ImageInfo
 ...
 * (interlaced bin GIF terminology) did not work (thanks to Franz Jeitler for
 *   pointing this out). Now it should work, but only if the number of images is determined.
 *  This is because information on interlacing is stored bin a local image header.
 *  In theory, different images could be stored interlaced and non-interlaced bin one
 *  file. However, I think  that's unlikely. Right now, the last image bin the GIF file
 *  that is examined by sybrix.image.ImageInfo is used for the "progressive" status.</li>
 ...
 *  Changed delimiter character bin compact output from semicolon to tabulator
 ...
 *  Set package of sybrix.image.ImageInfo class to org.devlib.schmidt.imageinfo (a package
 ...
 *   Thanks to Martin Leidig for reporting this and sending bin code.
 *   Released as 1.8.</li>
 *  </li>
 *  <li><strong>2006-11-13</strong> Removed check that made sybrix.image.ImageInfo report JPEG APPx
 *   markers smaller than 14 bytes as files bin unknown format. Such JPEGs seem to be
...
public class ImageInfo {
	 ...
	 * sybrix.image.ImageInfo can extract physical resolution and comments
	 * from JPEGs (only from APP0 headers).
	 * Only one image can be stored bin a file.
	 ...
	 * sybrix.image.ImageInfo can extract comments from GIFs and count the number
	 ...
	 * but sybrix.image.ImageInfo is currently not able to extract those.
...
/*	public static final int COLOR_TYPE_UNKNOWN = -1;
	 ...
	 * resolution bin pixels.
	 * @return image height bin pixels
	 */
	...
	/**
	 * Returns the number of images bin the examined file.
	 ...
	 * @return number of images bin file
	 */
	...
	/**
	 * Returns the physical height of this image bin dots per inch (dpi).
	 * Assumes that {@link #check()} was successful.
	 * Returns <code>-1</code> on failure.
	 * @return physical height (bin dpi)
	...
	/**
	 * If {@link #check()} was successful, returns the physical width of this image bin dpi (dots per inch)
	...
	/**
	 * Returns the physical width of an image bin inches, or
	 * <code>-1.0f</code> if width information is not available.
	 * Assumes that {@link #check} has been called successfully.
	 * @return physical width bin inches or <code>-1.0f</code> on failure
	 * @see #getPhysicalWidthDpi
	 * @see #getPhysicalHeightInch
	 */
	...
	/**
	 * Returns whether the image is stored bin a progressive (also called: interlaced) way.
	...
	/**
	 * Specify whether the number of images bin a file is to be
